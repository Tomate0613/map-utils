...
import dev.doublekekse.festlyutils.data.FestlyUtilsSavedData;
        ...
        dispatcher.register(
            literal("inv").requires(source -> source.hasPermission(2))
                .then(literal("save").then(argument("player", EntityArgument.player()).then(argument("save", StringArgumentType.string()).then(argument("remove", BoolArgumentType.bool()).executes(context -> {
                        ...
                        var data = FestlyUtilsSavedData.getServerData(context.getSource().getServer());
                        ...
                        context.getSource().sendSuccess(() -> Component.translatable("commands.festly-utils.inventory.saved", finalSave), false);
                    ...
                    if (invList == null) {
                        context.getSource().sendFailure(Component.translatable("commands.festly-utils.inventory.missing", finalSave));
                        return -1;
                    }

                    inv.load(invList);

                    context.getSource().sendSuccess(() -> Component.translatable("commands.festly-utils.inventory.loaded", finalSave), false);
